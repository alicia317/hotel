...
```powershell
    ...
    docker build --tag=juergenzimmermann/kunde:2024.04.0-azul .
    # Azul Zulu mit Ubuntu Jammy (2022.04) als Basis-Image
    docker build --tag=juergenzimmermann/kunde:2024.04.0-eclipse --file=Dockerfile.eclipse .
    # Eclipse Temurin mit UBI (Universal Base Image) von Red Hat als Basis-Image
    docker build --tag=juergenzimmermann/kunde:2024.04.0-eclipse-ubi --file=Dockerfile.eclipse-ubi .
```
...
```powershell
    docker inspect juergenzimmermann/kunde:2024.04.0-buildpacks
    docker inspect juergenzimmermann/kunde:2024.04.0-azul
    docker inspect juergenzimmermann/kunde:2024.04.0-eclipse
    docker inspect juergenzimmermann/kunde:2024.04.0-eclipse-ubi
```
...
```powershell
    docker sbom juergenzimmermann/kunde:2024.04.0-buildpacks
```
...
```powershell
    docker run --publish 8080:8080 `
      ...
      --name kunde --rm juergenzimmermann/kunde:2024.04.0-buildpacks
```
...
```powershell
    docker save juergenzimmermann/kunde:2024.04.0-buildpacks > kunde.tar
```
...
Um das Image mit dem Tag `2024.04.0-buildpacks` zu bauen, muss die Umgebungsvariable
`TAG` auf den Wert `2024.04.0-buildpacks` gesetzt sein. Dabei ist auf die Großschreibung
...
Außerdem generiert Skaffold noch ein SHA-Tag zusätzlich zu `2024.04.0-buildpacks`.
...
```powershell
    docker scout quickview juergenzimmermann/kunde:2024.04.0-buildpacks
    docker scout quickview juergenzimmermann/kunde:2024.04.0-azul
    docker scout quickview juergenzimmermann/kunde:2024.04.0-eclipse
    docker scout quickview juergenzimmermann/kunde:2024.04.0-eclipse-ubi
    docker scout quickview juergenzimmermann/kunde:2024.04.0-buildpacks-azul
    docker scout quickview juergenzimmermann/kunde:2024.04.0-buildpacks-eclipse
```
...
```powershell
    docker scout cves juergenzimmermann/kunde:2024.04.0-buildpacks
    docker scout cves --format only-packages juergenzimmermann/kunde:2024.04.0-buildpacks
````
